...
class Environment():
    ...
    def __init__(self, maze, display_width, display_height, max_moves, start_position=(1, 1)):
        ...
        self.SUB_GOALS = [(goal_x - 1, goal_y), (goal_x, goal_y - 1), (goal_x - 1, goal_y - 1),
                          (goal_x - 2, goal_y - 1)]
    ...
    def render(self, action, episode=-1, epsilon=-1, alpha=-1, gamma=-1):
        ...
        '''
        extra_width = 5
        display_matrix = np.zeros((self.HEIGHT, self.WIDTH + extra_width, 3),
                                  dtype=np.uint8)  # basically an image with 3 channels RGB
